...
- Live listing policies include `drafts.Users can manage own drafts` with role `public`, while repository migrations define authenticated draft policies. Verify whether the live `public` role is intentional and inspect the `USING`/`WITH CHECK` expressions.
- Live policy sets contain duplicate-looking account-data families such as `Users can manage account data` and `manage_account_data_non_recursive` on CRM tables. These likely reflect multiple migration generations or manual changes; compare exact expressions and grants.
- Service-role policies are reported with role `public` on several cache/log tables (`category_aspects_cache`, `category_hygiene_log`, `ebay_taxonomy_cache`, `lookup_decisions`, and `market_price_history`). This is a material security-review item because policy names do not prove the role is restricted to service-role callers.
- Anonymous insert/read policies exist on `audits`, `domain_requests`, `leads`, `smile_assessments`, and tenant onboarding objects. These are CRM or public-intake surfaces and are outside the default listing cohort.
- `storage.objects` has live policies for `client-uploads` that are absent from repository storage migrations.

The next RLS artifact must include exact `pg_policies` expressions, table grants, and an allow/deny test matrix for `anon`, `authenticated`, and `service_role`. No policy is considered reconciled from its name, role, or command alone.
