...
- **New minor finding, not yet fixed:** `market-watch-refresh` (the function that writes to `market_price_history`) has no `verify_jwt` override in `supabase/config.toml`, so it defaults to gateway `verify_jwt = true` — but that only requires _a_ validly-signed JWT, and the public anon key satisfies that. There's no additional code-level check, so anyone with the anon key can still trigger repeated refreshes (cost/abuse vector, not a data-integrity one now that the RLS fix above is in). Logged as a new item, not folded into RBR-0003's already-closed list since it wasn't flagged by the `verify_jwt = false` scan.
