...
function ConfidenceBadge({
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-medium ${cfg[confidence]}`}
    >
...
function PricingTab({
  ...
  const priceToApply = customPrice
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div
          className={`rounded-lg border p-4 text-center ${priceSuggestion.suggestedPrice ? "border-primary bg-primary/5" : "bg-muted/30"}`}
        >
          <p className="text-xs text-muted-foreground mb-1">Suggested Price</p>
          <p
            className={`text-2xl font-bold ${priceSuggestion.suggestedPrice ? "text-primary" : "text-muted-foreground"}`}
          >
            {priceSuggestion.suggestedPrice
      ...
      {priceSuggestion.suggestedPrice && (
        <div>
          ...
          {customPrice &&
            parseFloat(customPrice) !== priceSuggestion.suggestedPrice && (
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            {market.avgActivePrice && (
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  ${market.avgActivePrice.toFixed(2)}
                </span>
              </div>
            )}
            {market.minActivePrice && (
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  ${market.minActivePrice.toFixed(2)}
                </span>
              </div>
            )}
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {(market.sellThroughRate * 100).toFixed(0)}%
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Competition</span>
              <span
                className={`font-medium capitalize ${market.competitionLevel === "high" ? "text-red-600" : market.competitionLevel === "medium" ? "text-amber-600" : "text-emerald-600"}`}
              >
            ...
            <div className="flex justify-between">
              ...
              <span
                className={`font-medium capitalize ${market.demandSignal === "strong" ? "text-emerald-600" : market.demandSignal === "weak" ? "text-red-600" : "text-amber-600"}`}
              >
                {market.demandSignal}
              </span>
            </div>
      ...
      <div className="rounded-lg bg-muted/40 p-3 flex gap-2">
        ...
        <div className="flex-1 space-y-1">
          <div className="flex items-center gap-2">
            ...
            {priceSuggestion.estimatedImpact !== "Minimal" &&
              priceSuggestion.estimatedImpact !== "Unknown" && (
                <span className="text-xs font-medium text-primary">
                  {priceSuggestion.estimatedImpact}
                </span>
              )}
          </div>
    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          <span
            className={`text-xs font-medium ${titleLen > 80 ? "text-red-600" : titleLen < 30 ? "text-amber-600" : "text-emerald-600"}`}
          >
            {titleLen}/80 chars
          </span>
        </div>
      ...
      {titleSuggestion.issuesFound.length > 0 && (
        <div className="space-y-2">
          ...
          {titleSuggestion.issuesFound.map((issue, i) => (
            <div
              key={i}
              className="flex gap-2 text-sm bg-amber-50 border border-amber-200 rounded-lg p-3"
            >
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          <span
            className={`text-xs font-medium ${editableLen > 80 ? "text-red-600" : editableLen < 30 ? "text-amber-600" : "text-emerald-600"}`}
          >
        ...
        <div className="mt-2 flex gap-2">
          <Button
            ...
            disabled={
              ...
              editableTitle.trim() === listing.title.trim()
            }
            className="flex-1"
          >
            {applying ? (
    ...
    <div className="space-y-4">
      ...
      {descriptionSuggestion.issuesFound.length > 0 && (
        <div className="space-y-2">
          ...
          {descriptionSuggestion.issuesFound.map((issue, i) => (
            <div
              key={i}
              className="flex gap-2 text-sm bg-blue-50 border border-blue-200 rounded-lg p-3"
            >
        ...
        <div>
          ...
          <div className="rounded-lg border border-primary/30 bg-primary/5 p-3 max-h-[300px] overflow-y-auto">
            <p className="text-sm whitespace-pre-wrap italic text-muted-foreground mb-3 leading-relaxed">
              Note: Bulk description optimization is disabled for large sets to
              conserve API usage. This preview shows how we can restructure this
              specific listing for better conversion.
            </p>
      ...
      <div>
        ...
        <div className="mt-2 flex gap-2">
          <Button
            ...
            disabled={
              applying ||
              editableDescription.trim().length === 0 ||
              editableDescription.trim() === (listing.description || "").trim()
            }
            className="flex-1"
          >
            {applying ? (
      ...
      <div className="rounded-lg bg-muted/40 p-3 flex gap-2">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            {descriptionSuggestion.reasoning}
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border p-4">
        <div className="flex justify-between items-center mb-2">
          ...
          <span
            className={`text-lg font-bold ${strPct >= 50 ? "text-emerald-600" : strPct >= 20 ? "text-amber-600" : "text-red-600"}`}
          >
            {strPct}%
          </span>
        </div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          {market.soldCount.toLocaleString()} sold ·{" "}
          {market.activeCount.toLocaleString()} active
        </p>
      </div>
        ...
        <div className="space-y-2 text-sm">
          ...
          {market.minSoldPrice && market.maxSoldPrice && (
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium">
                ${market.minSoldPrice.toFixed(2)} – $
                {market.maxSoldPrice.toFixed(2)}
              </span>
            </div>
          )}
          {market.avgActivePrice && (
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-lg border p-3 text-center">
          ...
          <Badge
            variant="outline"
            className={`capitalize ${market.competitionLevel === "high" ? "border-red-300 text-red-700" : market.competitionLevel === "medium" ? "border-amber-300 text-amber-700" : "border-emerald-300 text-emerald-700"}`}
          >
            {market.competitionLevel}
          </Badge>
        </div>
        <div className="rounded-lg border p-3 text-center">
          ...
          <Badge
            variant="outline"
            className={`capitalize ${market.demandSignal === "strong" ? "border-emerald-300 text-emerald-700" : market.demandSignal === "weak" ? "border-red-300 text-red-700" : "border-amber-300 text-amber-700"}`}
          >
            {market.demandSignal}
          </Badge>
        </div>
...
export default function OptimizationModal({
  ...
  useEffect(() => {
    if (open && listing && !result) {
      analyze({
        ...
        setEditableTitle(
          analysis.titleSuggestion.suggestedTitle?.trim() ||
            listing.title ||
            "",
        );
  ...
  const handleApplyContent = async (mode: "title" | "description" | "both") => {
    ...
    const descriptionToSend =
      mode === "title" ? originalDescription : editableDescription;
          ...
          <div className="space-y-4">
            ...
            {analyzing && (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Fetching sold prices & competitor data
                </p>
              </div>
            )}
            ...
            {!analyzing && result && (
              <>
                  ...
                  <div className="flex items-start gap-2 text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg p-3">
                    ...
                    <div>
                      <p className="font-medium">
                        Limited pricing data right now.
                      </p>
                      <p className="text-amber-700">
                        Competitor comps may be incomplete, but the title and
                        description suggestions are still useful and can be
                        applied confidently.
                      </p>
                    </div>
                  </div>
                ...
                <Tabs defaultValue="pricing">
                  <TabsList
                    className={`grid w-full ${result.descriptionSuggestion.suggestedDescription || result.descriptionSuggestion.issuesFound.length > 0 ? "grid-cols-4" : "grid-cols-3"}`}
                  >
                    ...
                    {(result.descriptionSuggestion.suggestedDescription ||
                      result.descriptionSuggestion.issuesFound.length > 0) && (
                  ...
                  {(result.descriptionSuggestion.suggestedDescription ||
                    ...
                    <TabsContent value="description" className="mt-4">
                      <DescriptionTab
                        ...
                        onApplyDescription={() =>
                          handleApplyContent("description")
                        }
                        applying={applyingContent}
                      />
                    </TabsContent>
