...
export default function BottomNav() {
  ...
  const currentGroup = openGroup ? groups.find((g) => g.key === openGroup) : null;
      ...
      {currentGroup && (
        ...
        >
          <div className={`rounded-2xl border border-border bg-card shadow-xl overflow-hidden`}>
            {/* Sheet header */}
            <div className={`flex items-center justify-between px-4 py-3 ${currentGroup.bgColor} border-b border-border/50`}>
              <div className="flex items-center gap-2">
                <currentGroup.icon className={`w-4 h-4 ${currentGroup.color}`} />
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                      ...
                      <item.icon className={`w-4 h-4 ${active ? currentGroup.color : "text-muted-foreground"}`} />
                    </div>
                    <span className="text-sm font-medium flex-1">{item.label}</span>
                    {active && (
                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${currentGroup.bgColor} ${currentGroup.color}`}>
                        Active
                      </span>
                    )}
                    {!active && <ChevronRight className="w-3.5 h-3.5 text-muted-foreground/50" />}
                  </button>
                );
              })}
            </div>
        ...
        <div className="flex max-w-lg mx-auto h-full">
          ...
          {permanentTabs.map((tab) => {
            ...
            return (
              <button
                key={tab.path}
                onClick={() => { navigate(tab.path); setOpenGroup(null); }}
            ...
            return (
              <button
                ...
                className={`flex-1 flex flex-col items-center justify-center gap-0.5 transition-colors relative ${
                  active || isOpen ? group.color : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {/* Active dot indicator */}
                {active && !isOpen && (
                  <span className={`absolute top-2 right-1/4 w-1.5 h-1.5 rounded-full ${
