...
import {
  Zap, TrendingDown, TrendingUp, Clock, Type,
...
function FlagIcon({ flag }: { flag: OptimizationFlag }) {
  switch (flag) {
    case "overpriced": return <TrendingDown className="w-3.5 h-3.5 text-red-500" />;
    case "underpriced": return <TrendingUp className="w-3.5 h-3.5 text-emerald-500" />;
    case "stale": return <Clock className="w-3.5 h-3.5 text-orange-500" />;
    case "poor_title": return <Type className="w-3.5 h-3.5 text-purple-500" />;
  }
}
...
function ScoreBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`text-xs font-bold w-6 text-right ${score >= 70 ? "text-red-600" : score >= 40 ? "text-amber-600" : "text-emerald-600"}`}>
        {score}
      </span>
    </div>
  );
...
export default function OptimizationQueueWidget({ listings, onPriceApplied }: Props) {
    ...
    for (const listing of toScan) {
      ...
      if (result) {
        const daysActive = listing.listingDate
          ? Math.floor((Date.now() - new Date(listing.listingDate).getTime()) / (1000 * 60 * 60 * 24))
          : 0;
    ...
    setQueue((prev) =>
      prev.map((item) =>
        item.listing.listingId === listingId
          ? { ...item, listing: { ...item.listing, price: newPrice }, flags: [], opportunityScore: 0, direction: "keep" as const }
          : item
      )
    );
  ...
  const highPriorityCount = queue.filter((q) => q.opportunityScore >= 70).length;
  ...
  return (
    <div className="rounded-xl border bg-card">
      ...
      <div className="flex items-center justify-between p-4 border-b">
        ...
        {scanned && (
          <Button variant="ghost" size="sm" onClick={scanListings} disabled={scanning} className="h-8 text-xs">
      ...
      <div className="p-4">
        ...
        {!scanning && !scanned && (
          <div className="flex flex-col items-center py-4 text-center">
            ...
            <p className="text-sm font-medium mb-1">Find Optimization Opportunities</p>
            <p className="text-xs text-muted-foreground mb-4 max-w-xs">
              Analyzes your listings against current market data to surface pricing and title improvements.
            </p>
        ...
        {scanning && (
          <div className="flex flex-col items-center py-6 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Fetching market data for each listing</p>
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            {highPriorityCount > 0 && (
              <div className="flex items-center gap-2 p-2.5 rounded-lg bg-red-50 border border-red-200 mb-3">
                ...
                <p className="text-xs text-red-700 font-medium">
                  {highPriorityCount} high-priority listing{highPriorityCount > 1 ? "s" : ""} need immediate attention
                </p>
              </div>
            )}
            ...
            {queue.length === 0 && (
              <div className="text-center py-4 text-sm text-muted-foreground">
                No listings could be analyzed. Make sure your eBay account is connected.
              </div>
            )}
            ...
            {queue.slice(0, 5).map((item) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium truncate">{item.listing.title}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-muted-foreground">${item.listing.price.toFixed(2)}</span>
                    {item.suggestedPrice && item.direction !== "keep" && (
                      <span className={`text-xs font-medium ${item.direction === "lower" ? "text-red-600" : "text-emerald-600"}`}>
                    ...
                    <div className="flex gap-1 mt-1.5">
                      {item.flags.map((f) => (
                        <div key={f} className="flex items-center gap-0.5">
                          <FlagIcon flag={f} />
                          <span className="text-xs text-muted-foreground capitalize">{f.replace("_", " ")}</span>
                        </div>
                      ))}
                    </div>
                  )}
