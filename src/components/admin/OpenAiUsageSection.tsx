    ...
    <>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-card border border-border rounded-xl p-4 space-y-1">
          ...
          <p className="text-2xl font-bold text-foreground">{formatTokensInMillions(openai.inputTokens ?? 0)}</p>
        ...
        <div className="bg-card border border-border rounded-xl p-4 space-y-1">
          ...
          <p className="text-2xl font-bold text-foreground">{formatTokensInMillions(openai.outputTokens ?? 0)}</p>
          <p className="text-[10px] text-muted-foreground">
        ...
        <div className="bg-card border border-border rounded-xl p-4 space-y-1">
          ...
          <p className="text-2xl font-bold text-foreground">${(openai.estimatedCost ?? 0).toFixed(4)}</p>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={openai.last30DaysCost}>
                ...
                <Tooltip
                  ...
                  formatter={(value: unknown) => `$${parseFloat(String(value)).toFixed(6)}`}
                />
                <Line
                  type="monotone"
                  dataKey="cost"
                      ...
                      <td className="px-4 py-2 text-right font-semibold text-foreground">
                        ...
                        {openai.estimatedCost > 0 && (
                          <div className="text-[10px] text-muted-foreground">
                            {((stats.cost / openai.estimatedCost) * 100).toFixed(1)}%
                          </div>
                        )}
                      </td>
                    </tr>
