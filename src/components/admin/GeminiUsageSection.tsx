...
export function GeminiUsageSection({ gemini }: GeminiUsageSectionProps) {
  return (
    <>
      {/* AI Calls Bar Chart */}
      {gemini?.last30Days && Array.isArray(gemini.last30Days) && gemini.last30Days.length > 0 && (
      ...
      {gemini?.last30DaysCost && Array.isArray(gemini.last30DaysCost) && gemini.last30DaysCost.length > 0 && (
        <div className="bg-card border border-border rounded-xl p-4 space-y-3">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={gemini.last30DaysCost}>
                ...
                <Tooltip
                  ...
                  formatter={(value: unknown) => `$${parseFloat(String(value)).toFixed(4)}`}
                />
                <Line
                  type="monotone"
                  dataKey="cost"
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-card border border-border rounded-xl p-4 space-y-1">
          ...
          <p className="text-2xl font-bold text-foreground">{formatTokensInMillions(gemini.inputTokens ?? 0)}</p>
        ...
        <div className="bg-card border border-border rounded-xl p-4 space-y-1">
          ...
          <p className="text-2xl font-bold text-foreground">{formatTokensInMillions(gemini.outputTokens ?? 0)}</p>
          <p className="text-[10px] text-muted-foreground">
        ...
        <div className="bg-card border border-border rounded-xl p-4 space-y-1">
          ...
          <p className="text-2xl font-bold text-foreground">${gemini.estimatedCost.toFixed(4)}</p>
                      ...
                      <td className="px-4 py-2 text-right font-semibold text-foreground">
                        ...
                        {gemini.estimatedCost > 0 && (
                          <div className="text-[10px] text-muted-foreground">
                            {((stats.cost / gemini.estimatedCost) * 100).toFixed(1)}%
                          </div>
                        )}
                      </td>
                    </tr>
