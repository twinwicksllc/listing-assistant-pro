...
export function CompetitorDetailsModal({
  ...
  const maxCount = Math.max(
    ...(priceDistribution.map((b) => b.count) ?? [1]),
    1,
  );
          ...
          <div className="flex-1 min-w-0 pr-3">
            ...
            {searchQuery && (
              <p
                ...
                title={`eBay searched for: "${searchQuery}"`}
              >
        ...
        <div className="p-5 space-y-5 max-h-[calc(90vh-180px)] overflow-y-auto">
          {activeTab === "prices" && (
            <>
              ...
              {/* Position badge */}
              <div
                className={`flex items-center gap-2 px-3 py-2.5 rounded-xl ${positionBg}`}
              >
                <PositionIcon
                  className={`w-4 h-4 flex-shrink-0 ${positionColor}`}
                />
                <div className="flex-1 min-w-0">
                  <span className={`text-sm font-semibold ${positionColor}`}>
                    {positionLabel}
                  </span>
                  {deltaPct !== null && (
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                {[
                  ...
                  <div
                    key={label}
                    className="bg-secondary/40 rounded-lg py-2 px-1"
                  >
              ...
              <div className="flex items-center justify-between pt-1 border-t border-border">
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  <Users className="w-3.5 h-3.5" />
                  <span>
                    {competitorCount} comparable listing
                    {competitorCount !== 1 ? "s" : ""} found
                  </span>
                </div>
              ...
              {!canSeeComparable && (
                <div className="bg-primary/10 border border-primary/30 rounded-lg p-3 text-center">
                  ...
                  <p className="text-[10px] text-primary/70">
                    Upgrade to see AI-filtered comparable listings that are
                    truly comparable for pricing
                  </p>
                </div>
              )}
                  ...
                  {loadingComparable ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Analyzing comparable listings with AI...
                      </p>
                    </div>
                  ) : comparable.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-xs text-muted-foreground">
                        No truly comparable listings found
                      </p>
                    </div>
                  ) : (
