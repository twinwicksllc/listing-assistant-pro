...
export function PricingInsightsTable({
  ...
  const [pricingFilter, setPricingFilter] = useState<
    "all" | "overpriced" | "underpriced" | "stale"
  >("all");
  ...
  const getDelta = (listing: ListingWithCompetitor): number => {
    ...
    const basis =
      listing.competitor?.medianPrice ?? listing.competitor?.avgPrice;
    if (!basis) return 0;
  ...
  const isItemCoolingDown = useCallback(
    (listing: ListingWithCompetitor): boolean => {
  ...
  const filtered = useMemo(() => {
    ...
    return listings.filter((l) => {
      ...
      if (pricingFilter === "stale")
        return (
          getCacheFreshness(l) === "stale" || getCacheFreshness(l) === "none"
        );
      return true;
    });
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      if (sortField === "title") cmp = a.title.localeCompare(b.title);
        ...
        cmp =
          (getSuggestedPrice(a) ?? a.price) - (getSuggestedPrice(b) ?? b.price);
      } else if (sortField === "delta") {
        cmp = getDelta(a) - getDelta(b);
      } else if (sortField === "competitors") {
        cmp =
          (a.competitor?.competitorCount ?? 0) -
          (b.competitor?.competitorCount ?? 0);
      }
  ...
  const getCacheFreshness = (
    listing: ListingWithCompetitor,
  ...
  const handleRefreshItem = (listing: ListingWithCompetitor) => {
    ...
    if (isItemCoolingDown(listing)) {
      toast.info(
        "Data refreshed recently — please wait 30 minutes before refreshing again.",
        {
          duration: 3000,
        },
      );
  ...
  return (
    <div className="space-y-4 overflow-visible">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col items-end gap-0.5">
          ...
          {lastBulkRefreshAt && !bulkCooldownActive && (
            <span className="text-[10px] text-muted-foreground">
              Last bulk:{" "}
              {new Date(lastBulkRefreshAt).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              })}
            </span>
      ...
      {(() => {
        ...
        const overpricedCount = allListings.filter(
          (l) => getDelta(l) > 10,
        ).length;
        const underpricedCount = allListings.filter(
          (l) => getDelta(l) < -10,
        ).length;
        const staleCount = allListings.filter(
          (l) =>
            getCacheFreshness(l) === "stale" || getCacheFreshness(l) === "none",
        ).length;

        const pillBase =
          "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold cursor-pointer border transition-all select-none";
        const pillActive = (color: string) =>
          `${pillBase} ${color} border-current`;
        ...
        return (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() =>
                setPricingFilter(
                  pricingFilter === "overpriced" ? "all" : "overpriced",
                )
              }
              className={
            ...
            <button
              onClick={() =>
                setPricingFilter(
                  pricingFilter === "underpriced" ? "all" : "underpriced",
                )
              }
              className={
                pricingFilter === "underpriced"
            ...
            <button
              onClick={() =>
                setPricingFilter(pricingFilter === "stale" ? "all" : "stale")
              }
              className={
                pricingFilter === "stale"
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {sorted.map((listing) => {
              ...
              const isRefreshing = refreshing.has(
              ...
              return (
                ...
                >
                  ...
                  {(() => {
                    ...
                    const isCheaper =
                      suggested != null && suggested < listing.price;
                    const isHigher =
                      suggested != null && suggested > listing.price;
                    return (
                      <td className="px-3 py-3 text-right">
                        {suggested != null ? (
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            <button
                              ...
                              disabled={
                                isApplying || wasApplied || !listing.listingId
                              }
                              ...
                              title={
                                wasApplied
                                  ? "Applied!"
                                  : `Apply $${suggested.toFixed(2)} to eBay`
                              }
                            >
                              {isApplying ? (
                  ...
                  <td className="px-3 py-3 text-right">
                    {hasData ? (
                      <p className="text-xs text-muted-foreground">
                        ${listing.competitor!.minPrice!.toFixed(2)} – $
                        {listing.competitor!.maxPrice!.toFixed(2)}
                      </p>
                    ) : (
                      <p className="text-xs text-muted-foreground/60">—</p>
                    )}
                  ...
                  <td className="px-3 py-3 text-right">
                    {hasData ? (
                      <div
                        className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium ${positioning.color}`}
                      >
                  ...
                  <td className="px-3 py-3 text-right">
                    {hasData ? (
                      <div>
                        ...
                        <p
                          className={`text-[10px] ${freshnessInfo.className}`}
                          title={itemCooldownLabel(listing)}
                        >
                          {freshnessInfo.label}
                        </p>
                      </div>
                  ...
                  <td className="px-3 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button
                        ...
                        title={
                          coolingDown
                            ? itemCooldownLabel(listing) +
                              " — wait 30min to refresh"
