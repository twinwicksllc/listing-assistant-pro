...
export function useVideoFrameExtraction({ videoUrl, voiceNote }: UseVideoFrameExtractionProps) {
  ...
  const extractFramesClientSide = async (
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const onError = () => { cleanup(); reject(new Error("Could not load video metadata for frame extraction.")); };
      const cleanup = () => {
    ...
    if (duration <= 0) throw new Error("Video duration is invalid for frame extraction.");
    ...
    for (let i = 0; i < frameCount; i++) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const onError = () => { cleanup(); reject(new Error("Failed while seeking video for frame extraction.")); };
        const cleanup = () => {
  ...
  const mapExtractError = (err: unknown): { code: string; message: string } => {
    ...
    if (raw.includes("tainted") || raw.includes("cross-origin") || raw.includes("securityerror"))
      return { code: "cors_tainted_canvas", message: "Video frame extraction was blocked by browser security policy. Try re-uploading, or use photo capture as fallback." };
    if (raw.includes("metadata"))
      return { code: "metadata_load_failed", message: "Could not read video metadata. Please try a different video file." };
    if (raw.includes("seek"))
      return { code: "seek_failed", message: "Frame seeking failed during extraction. Retry, or use photo capture for now." };
    return { code: "extract_failed", message: (err as Error)?.message || "Frame extraction failed." };
  };
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("video-frame-extract", {
        body: {
          ...
          frames: clientFrames,
          telemetry: { source: "client-canvas", extractionMs, framesGenerated: clientFrames.length, userAgent: navigator.userAgent },
        },
      });
      ...
      setExtractFramesMessage(frames.length > 0 ? `Extracted ${frames.length} frame${frames.length !== 1 ? "s" : ""}. Ready for AI identification.` : "No frames returned.");
    } catch (err) {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("video-frame-extract", {
        body: {
          ...
          maxFrames: 6,
          strategy: "scene_change",
          telemetry: { source: "fallback-no-frames", reason: extractFramesErrorCode, userAgent: navigator.userAgent },
        },
      });
      if (error) throw error;
      const frames = Array.isArray(data?.frames) ? data.frames.filter((f: { url?: unknown }) => typeof f?.url === "string") : [];
      ...
      setExtractFramesMessage((err as Error)?.message || "Fallback extraction failed.");
    } finally {
      setExtractingFrames(false);
    }
  };
