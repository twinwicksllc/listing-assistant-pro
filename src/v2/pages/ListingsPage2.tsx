  ...
  const handleSavePrice = async (listing: EbayListing) => {
    ...
    try {
      ...
      if (error || !data?.success) {
        toast.error(
          `Could not update price: ${data?.error || error?.message || "Unknown error"}`,
        );
        return;
      }
      setListings((prev) =>
        prev.map((l) => (l.listingId === id ? { ...l, price: newPrice } : l)),
      );
              ...
              <div style={cardBody}>
                ...
                {filtered.length > 0 && (
                  ...
                  >
                    ...
                    {selectedIds.size > 0 && (
                      <span style={{ fontSize: "0.8125rem", color: "#9BA3AD" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          >
                            {listing.views30d > 0 && (
                              <span
                                style={pillStyle}
                                title="Views = buyers who clicked to your View Item page (last 30 days)"
                              >
                                👁 {listing.views30d.toLocaleString()} views
                                (30d)
                              </span>
                            )}
                            {listing.impressions30d > 0 && (
                              <span
                                style={{
                                  ...pillStyle,
                                  background: "rgba(110,117,128,0.07)",
                                  color: "#6E7580",
                                }}
                                title="Impressions = times your listing appeared in search results (last 30 days)"
                              >
                                📊 {listing.impressions30d.toLocaleString()}{" "}
                                impr (30d)
                              </span>
                            )}
                            ...
                            {listing.transactions30d > 0 && (
                              <span style={pillStyle}>
                                🛒 {listing.transactions30d} sold (30d)
                              </span>
                            )}
