...
export default function ProfitReportPage() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      const ebayToken =
        tokenData?.token ?? localStorage.getItem("ebay-user-token");
      if (!ebayToken) {
              ...
              {items.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Shipping */}
                    <span
                      className={`text-xs text-right whitespace-nowrap ${item.shippingCollected >= 0 ? "text-foreground" : "text-red-500"}`}
                    >
                      {item.shippingCollected >= 0 ? "+" : "−"}
                      {fmtMoney(Math.abs(item.shippingCollected))}
                    </span>
