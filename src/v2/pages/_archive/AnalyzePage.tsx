...
export default function AnalyzePage() {
      ...
      try {
        ...
        if (!cancelled) {
          setEbayTokenForPolicies(
            data?.token ?? localStorage.getItem("ebay-user-token"),
          );
        }
      } catch {
        if (!cancelled)
          setEbayTokenForPolicies(localStorage.getItem("ebay-user-token"));
      }
    })();
    return () => {
      cancelled = true;
  ...
  const handleGenerate = async () => {
    if (!canAnalyze) {
      toast.error(
        `Monthly analysis limit reached (${currentPlanLimits.analysisLimit}). Upgrade for more listings.`,
      );
      navigate("/billing");
      return;
    }
    ...
    try {
      ...
      if (user?.id) {
        try {
          ...
          if (tokenData?.token) {
            ...
            city = tokenData?.city ?? null;
            console.log(
              "AnalyzePage: no token but got location data from database",
              {
                postalCode,
                city,
              },
      ...
      const { data, error } = await supabase.functions.invoke("ebay-publish", {
        body: {
          ...
          imageUrls: user?.id
            ? await uploadListingImages(imageUrls, user.id)
            : imageUrls,
          condition,
          ebayCategoryId,
          itemSpecifics,
          fulfillmentPolicyId:
            selectedPolicies.fulfillmentPolicyId || undefined,
          paymentPolicyId: selectedPolicies.paymentPolicyId || undefined,
          returnPolicyId: selectedPolicies.returnPolicyId || undefined,
          bestOfferEnabled: bestOfferEnabled || undefined,
          bestOfferAutoAcceptPrice:
          ...
          try {
            ...
            if (jsonStart !== -1) {
              ...
              if (firstErr?.message) {
                ...
                const cleanMsg = firstErr.message
                  .replace(/<[^>]+>/g, "")
                  .split(".")[0]
                  .trim();
        ...
        <div className="relative rounded-xl overflow-hidden border border-border aspect-square bg-secondary">
          ...
          {imageUrls.length > 1 && (
            <>
              ...
              <button
                onClick={() =>
                  setActivePhoto((p) => (p + 1) % imageUrls.length)
                }
            ...
            <div className="space-y-1.5">
              ...
              {includeAiFooter && (
                <p className="text-[10px] text-muted-foreground italic bg-muted rounded-md px-2.5 py-1.5">
                  "Listing generated by Sovereign AI Assistant. All details
              ...
              <div className="space-y-2">
                ...
                <div className="bg-card border border-border rounded-lg divide-y divide-border">
                  {displaySpecifics.map(([key, value]) => {
                    const isRequired =
                      ebayMetadata?.requiredAspects?.includes(key);
                    const isSuggested =
                ...
                <div className="flex items-center justify-between bg-card border border-border rounded-lg px-3 py-2">
                  ...
                  >
                    ...
                    ebayMetadata.allowedConditions.length > 0
                  ...
                  >
                    ...
                    <div className="flex items-start gap-2 bg-accent/30 rounded-lg p-2.5">
                      ...
                      <p className="text-[10px] text-accent-foreground leading-relaxed">
                        <strong>Disclaimer:</strong> This is an AI-estimated
                        grade based on photo analysis only. It is NOT a
                        substitute for professional grading by PCGS, NGC, or
                        other certification services. Actual grade may differ.
                      </p>
                    </div>
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                            ...
                            toast.success(
                              `Grade ${suggestedGrade} applied to item specifics`,
                            );
                          }}
                ...
                <div className="space-y-3">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      onChange={(e) =>
                        setAuctionBuyItNowEnabled(e.target.checked)
                      }
                      className="h-4 w-4 rounded border-border accent-primary"
                    />
                    <span className="text-xs text-muted-foreground">
                  ...
                  {auctionBuyItNowEnabled && (
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        placeholder="0.00"
                        onChange={(e) =>
                          setAuctionBuyItNow(parseFloat(e.target.value) || 0)
                        }
                        className="w-full bg-card border border-border rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                      />
                    </div>
                ...
                onClick={() => {
                  exportListing(exportPlatform, exportFormat, {
                    ...
                    fulfillmentPolicyId:
                      selectedPolicies.fulfillmentPolicyId ?? undefined,
                    paymentPolicyId:
                      selectedPolicies.paymentPolicyId ?? undefined,
                    returnPolicyId:
                      selectedPolicies.returnPolicyId ?? undefined,
                  });
                  recordUsage("export");
                  const platformLabel =
                  ...
                  const formatLabel =
                    exportFormat === "csv"
                      ...
                      : exportFormat === "excel"
                        ...
                        : "Google Sheets";
                  toast.success(
                    `${platformLabel} listing exported as ${formatLabel}`,
                  );
                }}
