...
export default function ListingDetailModal({
  ...
  async function loadCogs() {
    ...
    try {
      ...
      if (skuList.length > 0 || listing.listingId) {
        ...
        if (skuList.length > 0)
          orParts.push(
            `ebay_sku.in.(${skuList.map((s) => `"${s}"`).join(",")})`,
          );
        if (listing.listingId)
          orParts.push(`ebay_listing_id.eq."${listing.listingId}"`);
        ...
        console.log("[ListingDetailModal] listing_cogs ->", {
          cogsRows,
          cogsErr,
          skuList,
      ...
      if (skuList.length > 0 || listing.listingId) {
        ...
        console.log("[ListingDetailModal] drafts fallback ->", {
          draftRows,
          draftErr,
          skuList,
        });
  ...
  const margin =
    grossProfit != null && price > 0 ? (grossProfit / price) * 100 : null;

  // Estimated eBay fees (~13.25% for most categories + $0.30 fixed)
  const estFees = price * 0.1325 + 0.3;
  const netProfit = cogs != null ? price - cogs - estFees : null;
  const netMargin =
    netProfit != null && price > 0 ? (netProfit / price) * 100 : null;

  // Offer evaluator
  const offerVal = parseFloat(offerInput);
  const offerNetProfit =
    !isNaN(offerVal) && cogs != null
      ? offerVal - cogs - (offerVal * 0.1325 + 0.3)
      : null;
  const offerMargin =
    offerNetProfit != null && offerVal > 0
    ...
    >
      <div
        style={{
          ...
          maxWidth: 680,
          boxShadow:
            "0 24px 80px rgba(0,30,80,0.22), 0 4px 16px rgba(0,0,0,0.08)",
          overflow: "hidden",
          position: "relative",
          marginBottom: "2rem",
        }}
        ...
        >
            ...
            {cogsLoading ? (
              ...
              >
                ...
                >
                  <AlertCircle
                    size={20}
                    style={{ color: COLORS.amber, flexShrink: 0, marginTop: 1 }}
                  />
                  <div>
                    <div
                      style={{
                    ...
                    >
                      Without a cost of goods, you can't calculate your true
                      profit or evaluate offers intelligently.
                    </div>
                  </div>
                </div>
              ...
              <div>
                ...
                >
                  ...
                  <StatCell
                    ...
                    sub={
                      margin !== null ? `${fmt(margin, 1)}% margin` : undefined
                    }
                ...
                {netProfit !== null && (
                  ...
                  >
                    {netProfit > 0 ? (
                      <>
                        <CheckCircle size={14} /> At list price you net $
                        {fmt(netProfit)} (after eBay fees)
                      </>
                    ) : netProfit === 0 ? (
                      <>
                      ...
                      <>
                        <XCircle size={14} /> Selling at list price loses $
          ...
          {cogs !== null && cogsRecord && (
            <div>
              ...
              >
                ...
                >
                  Got an offer on eBay? Enter the amount to see if you should
                  accept it.
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  {offerVal > 0 && offerNetProfit !== null && (
                    ...
                    >
                      {offerNetProfit > 0 ? (
                        <>
                          <CheckCircle size={14} /> Accept — net $
                          {fmt(offerNetProfit)} ({fmt(offerMargin!, 1)}% margin)
                        </>
                      ) : offerNetProfit === 0 ? (
                        <>
                        ...
                        <>
                          <XCircle size={14} /> Decline — lose $
                          {fmt(Math.abs(offerNetProfit))}
                        </>
                      )}
                    </div>
                  )}
                ...
                {cogs !== null && (
                  ...
                  >
                    ...
                    <strong style={{ color: COLORS.textPrimary }}>
                      ${fmt((cogs + 0.3) / (1 - 0.1325))}
                    </strong>
                    <span style={{ marginLeft: "0.4rem" }}>
                      (covers cost + eBay fees)
                    </span>
                  </div>
                )}
              </div>
          ...
          <div>
            ...
            >
              <StatCell
                label="Views 7d"
                value={listing.views7d.toLocaleString()}
              />
              <StatCell
                label="Views 30d"
                value={listing.views30d.toLocaleString()}
              />
              <StatCell
                label="Views 90d"
                value={listing.views90d.toLocaleString()}
              />
            </div>
            <div
              style={{
            ...
            >
              ...
              <StatCell
                ...
                sub={
                  listing.transactions90d > 0
                    ? `${listing.transactions90d} in 90d`
                    : undefined
                }
                color={
  ...
  return (
    <div>
      ...
      {preview !== null && (
        ...
        >
          {preview >= 0 ? <TrendingUp size={13} /> : <TrendingDown size={13} />}
          ...
          >
            (
            {listingPrice > 0 ? ((preview / listingPrice) * 100).toFixed(1) : 0}
            % margin)
          </span>
        </div>
      )}
