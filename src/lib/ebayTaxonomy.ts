...
export function clearCache(): void {
  try {
    ...
    const ebayKeys = keys.filter(
      (key) =>
        key.startsWith("ebay_category_") || key.startsWith("ebay_aspects_"),
    );
 ...
 */
async function fetchTaxonomyAPI<T>(
  endpoint: string,
  token: string,
): Promise<T> {
  ...
  try {
    ...
    throw new Error(
      `Unexpected error fetching from eBay Taxonomy API: ${error}`,
    );
  }
}
...
export async function getCategorySuggestions(
  ...
  try {
    console.log(
      `[API Call] Fetching category suggestions for "${productDescription}"`,
    );
    ...
    if (
      ...
      Array.isArray(response.categorySuggestions)
    ) {
...
export async function getRequiredAspects(
  ...
  // Try cache first
  const cached = getCachedData<AspectRequirements>(
    cacheKey,
    CACHE_TTL_ASPECTS_HOURS,
  );
  if (cached) {
