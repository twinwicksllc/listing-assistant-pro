...
function TrendBadge({ listing }: { listing: EbayListing }) {
  ...
  return (
    <span
      className={`flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded-md font-medium ${cls}`}
    >
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className="flex items-center gap-0.5 text-muted-foreground">
          ...
          <span className="font-semibold text-foreground ml-0.5">
            {listing.views7d.toLocaleString()}
          </span>
        </span>
        {paceArrow}
        <span className="flex items-center gap-0.5 text-muted-foreground">
        ...
        <span className="flex items-center gap-0.5 text-muted-foreground">
          ...
          <span className="font-semibold text-foreground ml-0.5">
            {listing.views90d.toLocaleString()}
          </span>
        </span>
...
function InlinePriceEditor({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error || !data?.success) {
        toast.error(
          `Price update failed: ${data?.error || error?.message || "Unknown error"}`,
        );
      } else {
  ...
  if (!editing) {
    return (
      <button
        onClick={() => {
          setValue(listing.price.toFixed(2));
          setEditing(true);
        }}
...
function BulkPriceModal({
  ...
  const [adjustMode, setAdjustMode] = useState<"fixed" | "pct" | "amount">(
    "fixed",
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (successCount > 0)
        toast.success(
          `${successCount} listing${successCount !== 1 ? "s" : ""} updated successfully`,
        );
      if (failCount > 0) {
        const failed = results?.filter((r: any) => !r.success) || [];
        toast.error(
          `${failCount} update${failCount !== 1 ? "s" : ""} failed: ${failed[0]?.error || "Unknown error"}`,
        );
      }
      const successfulUpdates = updates.filter(
        (u, i) => results?.[i]?.success !== false,
      );
      onSuccess(
      ...
      <div className="bg-card border border-border rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col">
        ...
        <div className="px-5 py-3 border-b border-border bg-secondary/30">
          ...
          <div className="flex gap-2">
            ...
            <input
              ...
              placeholder={
                adjustMode === "pct" ? "e.g. 10 or -5" : "e.g. 49.99"
              }
          ...
          <p className="text-[10px] text-muted-foreground mt-1.5">
            ...
            {adjustMode === "pct" &&
              "Raise (+) or lower (-) by percentage. e.g. -10 = 10% off"}
            {adjustMode === "amount" &&
              "Add (+) or subtract (-) a dollar amount from each price"}
          </p>
        </div>
              ...
              <div key={k} className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <input
                    ...
                    value={prices[k] || ""}
                    onChange={(e) =>
                      setPrices((prev) => ({ ...prev, [k]: e.target.value }))
                    }
        ...
        <div className="px-5 py-4 border-t border-border flex gap-3">
          ...
          >
            ...
            {saving
              ? "Updating..."
              : `Update ${selected.length} Price${selected.length !== 1 ? "s" : ""}`}
          </button>
        </div>
      </div>
    </div>
...
function StatPill({
  ...
  return (
    <span
      className={`flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-md ${highlight ? "bg-primary/10 text-primary font-semibold" : "text-muted-foreground"}`}
      title={label}
    >
...
export default function DashboardPage() {
  ...
  const [salesProfitWindow, setSalesProfitWindow] = useState<
    "7d" | "30d" | "90d"
  >("30d");
  ...
  const [cogsByListingDb, setCogsByListingDb] = useState<
    Record<string, number>
  >({}); // COGS from listing_cogs table
  ...
  const [listingViewMode, setListingViewMode] = useState<"cards" | "pricing">(
    "cards",
  );
  ...
  const fetchListings = useCallback(async () => {
    ...
    try {
      ...
      if (typeof data.orderCount30d === "number")
        setOrderCount30d(data.orderCount30d);
      if (typeof data.orderCount7d === "number")
        setOrderCount7d(data.orderCount7d);
      if (typeof data.orderCount90d === "number")
        setOrderCount90d(data.orderCount90d);
      ...
      if (user?.id && data.financial) {
        try {
          ...
          if (soldOrders.length > 0) {
            ...
            if (skus.length > 0)
              orParts.push(`ebay_sku.in.(${skus.join(",")})`);
        ...
        try {
          ...
          if (listingIds.length > 0) {
            const { data: cpData } = await supabase
              ...
              .select(
                "ebay_listing_id, avg_price, min_price, max_price, median_price, price_delta, competitor_count, price_distribution, fetched_at, expires_at",
              )
              .eq("user_id", user.id)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const orParts: string[] = [];
        if (skus.length > 0)
          orParts.push(`ebay_sku.in.(${skus.map((s) => `"${s}"`).join(",")})`);
        if (listingIds.length > 0)
          orParts.push(
  ...
  const filteredListings = useMemo(() => {
    ...
    if (filterTrend !== "all")
      list = list.filter((l) => getTrend(l) === filterTrend);
  ...
  const selectedListings = filteredListings.filter((l) =>
    selectedIds.has(listingKey(l)),
  );
  ...
  const handlePriceSaved = (
    ...
    setListings((prev) =>
      prev.map((l) => {
        ...
        if (listingId && l.listingId === listingId)
    ...
    setListings((prev) =>
      prev.map((l) => {
        const match = updates.find(
          (u) =>
            (u.offerId && u.offerId === l.offerId) ||
            (u.listingId && u.listingId === l.listingId),
        );
  ...
  const totalWatches = listings.reduce(
    (sum, l) => sum + (l.watchCount || 0),
    0,
  );
  ...
  const draftValue = drafts.reduce(
    (sum, d) => sum + (d.priceMin + d.priceMax) / 2,
    0,
  );
  ...
  const cogsByListing = useMemo(() => {
    const map: Record<
      string,
      { cogs: number | undefined; listingPrice: number; domain?: string }
    > = {};

    // First, add from drafts (has authoritative listingPrice)
    for (const d of drafts) {
      const breadcrumb = d.ebayCategoryBreadcrumb ?? "";
      const domain = /coins?|bullion|gold bar|silver bar|precious metal/i.test(
        breadcrumb,
      )
        ? "coins_bullion"
        : undefined;
    ...
    for (const listing of listings) {
      const dbCogs =
        (listing.sku ? cogsByListingDb[listing.sku] : undefined) ||
  ...
  const atRiskListings = spotPrices
    ? listings.flatMap((listing) => {
        ...
        if (
          !draft ||
          !draft.metalType ||
          draft.metalType === "none" ||
          !(draft.metalWeightOz ?? 0)
        )
          return [];
              ...
              {ebayAccount ? (
                <p className="text-xs text-muted-foreground">
                  ...
                  <span className="font-medium text-foreground">
                    {ebayAccount.businessName || ebayAccount.username}
                  </span>
                </p>
              ) : (
                <p className="text-xs text-muted-foreground">
      ...
      <div className="px-5 md:px-8 lg:px-12 max-w-3xl mx-auto space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-card border border-border rounded-xl p-4 space-y-1">
            ...
            <p className="text-xl font-bold text-foreground">
              ${(liveValue + draftValue).toFixed(2)}
            </p>
          ...
          <div className="bg-card border border-border rounded-xl p-4 space-y-1">
            ...
            <p className="text-xl font-bold text-foreground">
              {totalTransactions30d.toLocaleString()}
            </p>
            <div className="flex gap-2 text-[10px] text-muted-foreground flex-wrap">
              <span className="flex items-center gap-0.5">
                <span className="opacity-60">7d</span>{" "}
                <span className="font-medium text-foreground">
                  {orderCount7d.toLocaleString()}
                </span>
              </span>
              <span className="text-muted-foreground/40">·</span>
              <span className="flex items-center gap-0.5">
        ...
        {(fin30.revenue > 0 || fin90.revenue > 0 || loading) && (
          <div className="bg-card border border-border rounded-xl overflow-hidden">
              ...
              <div className="flex items-center gap-1 text-[10px]">
                {(
                  ...
                  <button
                    key={label}
                    onClick={() =>
                      setSalesProfitWindow(label as "7d" | "30d" | "90d")
                    }
            ...
            {(() => {
              const fin =
                salesProfitWindow === "7d"
                  ? fin7
                  : salesProfitWindow === "90d"
                    ? fin90
                    : fin30;
              const profitColor =
                fin.netProfit >= 0
              ...
              const profitBg =
                fin.netProfit >= 0 ? "bg-emerald-500/10" : "bg-red-500/10";
              return (
                <>
                  {/* Big profit number */}
                  <div className="px-4 pb-3">
                    <p className={`text-2xl font-bold ${profitColor}`}>
                      {fmtMoney(fin.netProfit)}
                    </p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      Net profit ({salesProfitWindow}) · {fin.orders} sale
                      {fin.orders !== 1 ? "s" : ""}
                    </p>
                  </div>
                  ...
                  <div className="border-t border-border divide-y divide-border">
                    ...
                    <div
                      className={`flex items-center justify-between px-4 py-2.5 ${profitBg}`}
                    >
                      <div className="flex items-center gap-2 text-xs font-semibold text-foreground">
                        <TrendingUp
                          className={`w-3.5 h-3.5 flex-shrink-0 ${profitColor}`}
                        />
                      ...
                      <div className="text-right">
                        ...
                        {fin.revenue + fin.shippingCollected > 0 && (
                          <span className="text-[10px] text-muted-foreground ml-1.5">
                            {(
                              (fin.netProfit /
                                (fin.revenue + fin.shippingCollected)) *
                              100
                            ).toFixed(1)}
                            % margin
                          </span>
                  ...
                  <div className="px-4 py-2.5 bg-secondary/30 flex items-center gap-3 flex-wrap">
                    ...
                    {[
                      ...
                      >
                        ...
                        <span
                          className={`font-semibold ml-0.5 ${val >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-500"}`}
                        >
                          {fmtMoneyShort(val)}
                        </span>
                      </span>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-amber-700 dark:text-amber-300">
                  {atRiskListings.length} listing
                  {atRiskListings.length !== 1 ? "s" : ""} below melt floor
                </span>
              </div>
              {meltAlertOpen ? (
            ...
            {meltAlertOpen && (
              <div className="px-4 pb-3 space-y-2">
                <p className="text-xs text-amber-700/80 dark:text-amber-300/80">
                  Spot prices have moved — these listings are priced below their
                  precious metal melt value.
                </p>
                {atRiskListings.map(({ listing, meltFloor, delta }) => (
                  <div
                    key={listing.offerId || listing.listingId}
                    className="flex items-center justify-between gap-2 text-xs bg-amber-500/10 rounded-lg px-3 py-2"
                  >
                    <p className="text-foreground font-medium line-clamp-1 flex-1">
                      {listing.title}
                    </p>
                    <div className="flex-shrink-0 text-right space-y-0.5">
                      <p className="text-amber-700 dark:text-amber-300 font-semibold">
                        Listed ${listing.price.toFixed(2)} · Melt $
                        {meltFloor.toFixed(2)}
                      </p>
        ...
        {needsAuth && (
          <div className="bg-accent/50 border border-accent rounded-xl p-4 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Connect your eBay account in Settings to see listings and
                traffic data.
              </p>
              <button
                onClick={() => navigate("/settings?tab=integrations")}
                  ...
                  <div className="flex border border-border rounded-lg overflow-hidden text-xs">
                    {(["all", "active", "inactive"] as const).map((s) => (
                      <button
                        ...
                        className={`flex-1 py-1.5 capitalize transition-colors ${filterStatus === s ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary"}`}
                      >
                        {s}
                      </button>
                    ))}
              ...
              {planFeatures.hasListingAnalytics ? (
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {[
                      {
                        label: "Zero views",
                        active: filterMaxViews === "0" && filterMinViews === "",
                        action: () => {
                          setFilterMinViews("");
                          setFilterMaxViews(filterMaxViews === "0" ? "" : "0");
                        },
                      },
                      {
                        label: "Has views",
                        active: filterMinViews === "1" && filterMaxViews === "",
                        action: () => {
                          setFilterMinViews(filterMinViews === "1" ? "" : "1");
                          setFilterMaxViews("");
                        },
                      },
                      {
                        label: "Has watchers",
                        active: filterMinWatchers === "1",
                        action: () =>
                          setFilterMinWatchers(
                            filterMinWatchers === "1" ? "" : "1",
                          ),
                      },
                      {
                        label: "Has sales",
                        active: filterHasSales,
                        action: () => setFilterHasSales(!filterHasSales),
                      },
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        min="0"
                        placeholder="∞"
                        value={filterMaxViews}
                        onChange={(e) => setFilterMaxViews(e.target.value)}
                        className="w-full text-xs border border-border rounded-lg px-2 py-1.5 bg-background focus:outline-none focus:ring-1 focus:ring-primary"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        min="0"
                        placeholder="0"
                        value={filterMinWatchers}
                        onChange={(e) => setFilterMinWatchers(e.target.value)}
                        className="w-full text-xs border border-border rounded-lg px-2 py-1.5 bg-background focus:outline-none focus:ring-1 focus:ring-primary"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        min="0"
                        placeholder="0"
                        value={filterMinImpressions}
                        onChange={(e) =>
                          setFilterMinImpressions(e.target.value)
                        }
                        className="w-full text-xs border border-border rounded-lg px-2 py-1.5 bg-background focus:outline-none focus:ring-1 focus:ring-primary"
                      />
                    </div>
                ...
                <div className="bg-muted/50 border border-border rounded-lg px-4 py-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Upgrade to Pro ($49/mo) to filter by views, watchers,
                    trends, and more.
                  </p>
                </div>
              )}
            ...
            <PricingInsightsTable
              ...
              onRefreshCompetitor={async (listingId) => {
                ...
                try {
                  const { data, error } = await supabase.functions.invoke(
                    "ebay-competitor-search",
                    {
                      body: {
                        userId: user.id,
                        listingId,
                        title: listings.find((l) => l.listingId === listingId)
                          ?.title,
                        categoryId: listings.find(
                          (l) => l.listingId === listingId,
                        )?.categoryId,
                        yourPrice: listings.find(
                          (l) => l.listingId === listingId,
                        )?.price,
                      },
                    },
                  ...
                  if (error) {
                    toast.error(
                      "Could not retrieve competitor prices. Please try again.",
                    );
                    return;
                  }
                  if (data?.error) {
              ...
              onRefreshAll={async () => {
                ...
                if (
                  ...
                  const diff =
                    new Date(bulkRefreshCooldownUntil).getTime() - Date.now();
                  const h = Math.floor(diff / 3600000);
                  const m = Math.ceil((diff % 3600000) / 60000);
                  toast.info(
                    `Bulk refresh available in ${h > 0 ? `${h}h ` : ""}${m}m`,
                    { duration: 4000 },
                  );
                  return;
                }
                ...
                if (staleListings.length === 0) {
                  toast.info(
                    "All listings have fresh data — nothing to refresh.",
                  );
                  return;
                }
                const cooldownUntil = new Date(
                  Date.now() + BULK_REFRESH_COOLDOWN_MS,
                ).toISOString();
                setBulkRefreshCooldownUntil(cooldownUntil);
                setLastBulkRefreshAt(new Date().toISOString());
                try {
                  localStorage.setItem(
                    BULK_REFRESH_COOLDOWN_KEY,
                    cooldownUntil,
                  );
                } catch {
                  /* ignore */
                ...
                toast.info(
                  `Refreshing ${staleListings.length} listing${staleListings.length !== 1 ? "s" : ""}…`,
                  { duration: 3000 },
                );
                let updated = 0;
                let failed = 0;
                ...
                if (failed === 0) {
                  ...
                  toast.warning(
                    `Updated ${updated}, failed ${failed}. Try refreshing failed items individually.`,
                  );
                }
              }}
              ...
              onApplyPrice={async (
                ...
                if (error || !data?.success) {
                  ...
                  throw new Error(
                    data?.error || error?.message || "reprice failed",
                  );
                }
                setListings((prev) =>
                  prev.map((l) =>
                ...
                toast.success(
                  `Price updated to $${newPrice.toFixed(2)} on eBay`,
                );
              }}
              userToken={ebayToken}
              userId={user?.id || ""}
                ...
                <div className="space-y-2">
                  {filteredListings.map((listing) => {
                    ...
                    return (
                      <div
                        key={k}
                        className={`bg-card border rounded-xl p-3 flex gap-3 transition-colors ${isSelected ? "border-primary/50 bg-primary/5" : "border-border"}`}
                      >
                        {/* Checkbox */}
                        <button
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-start gap-2 flex-wrap">
                            ...
                            <div className="flex items-center gap-1 flex-shrink-0">
                              ...
                              {planFeatures.hasCogsTracking &&
                                (() => {
                                  ...
                                  return cogsEntry?.cogs != null ? (
                                    <ProfitBadge
                                      listingPrice={
                                        cogsEntry.listingPrice || listing.price
                                      }
                                      cogs={cogsEntry.cogs}
                              ...
                              <span
                                className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${statusColor(listing.status)}`}
                              >
                          ...
                          {planFeatures.hasListingAnalytics ? (
                            <div className="flex flex-wrap items-center gap-1 mt-1">
                              ...
                              {listing.transactions30d > 0 && (
                                <StatPill
                                  icon={ShoppingCart}
                                  value={`${listing.transactions7d}/${listing.transactions30d}/${listing.transactions90d}`}
                                  label="Sales 7d/30d/90d"
                                  highlight
                                />
                              )}
