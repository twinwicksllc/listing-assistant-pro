...
export default function AnalyzePage() {
  ...
  const handleExtractFrames = async () => {
    ...
    try {
      ...
      setExtractFramesMessage(
        frames.length > 0
          ? `Extracted ${frames.length} frame${frames.length !== 1 ? "s" : ""}. Slice 1 uses mock frames; Slice 2 will add real extraction.`
          : "No frames returned.",
      );
    } catch (err: any) {
      setExtractFramesMessage(err?.message || "Frame extraction failed.");
      setExtractedFrames([]);
    } finally {
