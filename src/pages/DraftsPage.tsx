...
export default function DraftsPage() {
  ...
  const selectableDrafts = drafts.filter((d) => {
    ...
    return terminalVideoStatuses.has(
      String(d.ebayVideoStatus || "").toUpperCase(),
    );
  });
  const allSelected =
      ...
      <div className="px-4 space-y-3 max-w-lg mx-auto">
        ...
        {drafts.map((draft) => {
          ...
          const hasVideoProcessing =
            !!draft.ebayVideoId &&
            !terminalVideoStatuses.has(
              String(draft.ebayVideoStatus || "").toUpperCase(),
            );
              ...
              <button
                onClick={() => {
                  if (hasVideoProcessing) {
                    toast.info(
                      "Video still processing — wait for it to finish before publishing",
                    );
                    return;
                  }
