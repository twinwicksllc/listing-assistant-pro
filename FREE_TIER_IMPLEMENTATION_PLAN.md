...
| **OQ-15** | ✅ RESOLVED | `ebay-user` in production without identity scope | No scope change needed for `ebay-user` itself — the scope addition in `get_auth_url` affects new token grants only. **Correction (2026-09-25):** the original "confirmed working" claim here predated a real bug — `ebay-user/index.ts` built its Identity API request against `api.ebay.com` instead of `apiz.ebay.com`, 502ing on every call. Fixed by PR #629 (`identityApiBaseFor()`); confirmed live (200 OK, no errors) post-fix. See §3.0.1 and CLAUDE.md's eBay integration surface notes. |
