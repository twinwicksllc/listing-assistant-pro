...
const [aspectValidation, setAspectValidation] =
...
const handleCategoryDiscovery = async () => {
  ...
  try {
    ...
    const requirements = await getRequiredAspects(
      selected.categoryId,
      ebayToken,
    );
    ...
    const message =
      error instanceof Error ? error.message : "Failed to discover category";
    toast.error(message);
  } finally {
    setLoadingCategory(false);
  }
...
const validateItemAspects = async () => {
  ...
  try {
    ...
    if (!result.isValid) {
      const errors = [
        ...
        ...result.invalidValues.map(
          (inv) =>
            `${inv.aspectName}: "${inv.providedValue}" not in allowed values`,
        ),
      ];
    ...
    if (result.missingSuggested.length > 0) {
      toast.info(`Consider filling in: ${result.missingSuggested.join(", ")}`);
    }
            ...
            {categorySuggestions.slice(1).map((cat) => (
              <button
                ...
                onClick={async () => {
                  ...
                  const reqs = await getRequiredAspects(
                    cat.categoryId,
                    ebayToken!,
                  );
                  setAspects(reqs);
