...
test('Detects PCGS coin category (11981)', () => {
  const coinCategories = new Set([11981, 39464, 11980, 11971, 41099, 41102, 11973, 39455, 41084, 11950, 41111, 166679, 41109, 526, 45243, 39471, 39472, 39473, 39474, 39475]);
  assert(coinCategories.has(11981), 'PCGS category 11981 not in coin set');
});

test('Detects Morgan Dollar category (39464)', () => {
  const coinCategories = new Set([11981, 39464, 11980, 11971, 41099, 41102, 11973, 39455, 41084, 11950, 41111, 166679, 41109, 526, 45243, 39471, 39472, 39473, 39474, 39475]);
  assert(coinCategories.has(39464), 'Morgan Dollar category 39464 not in coin set');
});

test('Rejects parent category 253 (Coins: US)', () => {
  const coinCategories = new Set([11981, 39464, 11980, 11971, 41099, 41102, 11973, 39455, 41084, 11950, 41111, 166679, 41109, 526, 45243, 39471, 39472, 39473, 39474, 39475]);
  assert(!coinCategories.has(253), 'Parent category 253 should NOT be in coin set (Phase 1 requirement)');
});

test('Excludes non-coin categories', () => {
  const coinCategories = new Set([11981, 39464, 11980, 11971, 41099, 41102, 11973, 39455, 41084, 11950, 41111, 166679, 41109, 526, 45243, 39471, 39472, 39473, 39474, 39475]);
  assert(!coinCategories.has(3574), 'Electronics category 3574 should NOT be in coin set');
  assert(!coinCategories.has(15687), 'Sports category 15687 should NOT be in coin set');
});
...
test('Accepts PCGS MS 65 format', () => {
  ...
  assert(gradePattern.test('MS65'), 'MS65 (no space) should match grade pattern');
  assert(gradePattern.test('PR 70 DCAM'), 'PR 70 DCAM should match grade pattern');
});
...
test('Rejects invalid grading company', () => {
  const allowed = ['PCGS', 'NGC', 'ANACS', 'ICG', 'CAC', 'ICCS'];
  assert(!allowed.includes('FAKE_GRADER'), 'Invalid company should be rejected');
...
test('Accepts Extremely Fine to About Uncirculated tier', () => {
  const allowed = ['Uncirculated', 'Extremely Fine to About Uncirculated', 'Fine to Very Fine', 'Below Fine'];
  assert(allowed.includes('Extremely Fine to About Uncirculated'), 'EF to AU should be allowed');
});
...
test('Rejects non-standard condition (Acceptable)', () => {
  const allowed = ['Uncirculated', 'Extremely Fine to About Uncirculated', 'Fine to Very Fine', 'Below Fine'];
  assert(!allowed.includes('Acceptable'), 'Acceptable should NOT be allowed (eBay restricted)');
});

test('Rejects non-standard condition (Refurbished)', () => {
  const allowed = ['Uncirculated', 'Extremely Fine to About Uncirculated', 'Fine to Very Fine', 'Below Fine'];
  assert(!allowed.includes('Refurbished'), 'Refurbished should NOT be allowed (eBay restricted)');
});
...
test('Graded coin structure (complete)', () => {
  ...
  assert(['PCGS', 'NGC', 'ANACS', 'ICG', 'CAC', 'ICCS'].includes(graded.gradingCompany), 'Company must be valid');
  assert(graded.grade.match(/^[A-Z]{1,3}\s*\d{1,2}(\s+[A-Z]{2,})?$/), 'Grade must match pattern');
});
...
test('Graded coin structure (with cert)', () => {
  ...
  assert(graded.certificationNumber && graded.certificationNumber.length >= 3, 'Cert must be >= 3 chars if present');
});
...
test('Raw coin structure (complete)', () => {
  ...
  assert(['Uncirculated', 'Extremely Fine to About Uncirculated', 'Fine to Very Fine', 'Below Fine'].includes(raw.rawCondition), 'Condition must be one of 4 tiers');
});
...
test('Non-coin category does NOT require condition detail', () => {
  ...
  assert(isCoin && !hasCondition ? false : true, 'Should pass if non-coin (condition not required)');
});

test('Coin category with graded condition passes', () => {
  const isCoin = true;
  const condition = { type: 'graded', gradingCompany: 'PCGS', grade: 'MS 65' };
  assert(isCoin && condition && condition.type === 'graded', 'Should pass if coin with graded condition');
});

test('Coin category with raw condition passes', () => {
  const isCoin = true;
  const condition = { type: 'raw', rawCondition: 'Uncirculated' };
  assert(isCoin && condition && condition.type === 'raw', 'Should pass if coin with raw condition');
});
...
test('Clear error for missing coin condition', () => {
  const errorMsg = 'Coin condition details are REQUIRED for this category per eBay June 2026 mandate';
  assert(errorMsg.length > 0, 'Error message should be non-empty');
  assert(errorMsg.toLowerCase().includes('required'), 'Error should mention requirement');
  assert(errorMsg.toLowerCase().includes('coin'), 'Error should mention coin');
});

test('Clear error for invalid grading company', () => {
  const errorMsg = 'Invalid grading company: INVALID_CO. Must be one of: PCGS, NGC, ANACS, ICG, CAC, ICCS';
  assert(errorMsg.includes('Invalid grading company'), 'Error should specify company issue');
  assert(errorMsg.includes('PCGS, NGC'), 'Error should list valid options');
});

test('Clear error for invalid grade format', () => {
  const errorMsg = 'Grade format is invalid: "65 MS". Must be LETTER_CODE + NUMBER (e.g., "MS 65", "PR 70 DCAM")';
...
test('Clear error for invalid raw condition', () => {
  const errorMsg = 'Invalid raw condition: "Used". Must be one of: Uncirculated, Extremely Fine to About Uncirculated, Fine to Very Fine, Below Fine';
  assert(errorMsg.includes('Invalid raw condition'), 'Error should specify condition issue');
  assert(errorMsg.includes('Uncirculated'), 'Error should list valid options');
});

test('Transient error mentions retry', () => {
  const errorMsg = 'This may be a temporary service issue. Please try again or contact support.';
  assert(errorMsg.includes('try again'), 'Transient error should suggest retry');
  assert(errorMsg.includes('contact support'), 'Error should provide support contact info');
});
...
const statusText = failCount === 0 ? 'ALL TESTS PASSED ✓' : `${failCount} TESTS FAILED ✗`;

console.log(`${statusColor}${statusText}${colors.reset}`);
console.log(`${colors.bright}Total: ${passCount}/${totalTests} passed${colors.reset}`);
