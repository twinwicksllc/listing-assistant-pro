  ...
  try {
    ...
    if (action === "single_update") {
      ...
      if (offerId) {
        ...
        if (shouldFallback) {
          const msg =
            `[ebay-reprice] Inventory API error for ${offerId}: "${errorMsg}". ` +
            `Attempting fallback to Trading API with listing ${listingId}`;
          console.log(msg);
          try {
            ...
            const status = tradingResult.success ? "SUCCESS" : `FAILED: ${tradingResult.error}`;
            console.log(`[ebay-reprice] Trading API fallback result: ${status}`);
            return new Response(
        ...
        console.log(`[ebay-reprice] Inventory API result for ${offerId}: ${status}`);
        return new Response(
