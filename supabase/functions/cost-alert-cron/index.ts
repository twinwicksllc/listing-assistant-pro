...
serve(async (req) => {
  ...
  try {
    ...
    if (totalCost < COST_THRESHOLD) {
      console.log(
        `[COST-ALERT-CRON] Below threshold ($${COST_THRESHOLD}). No alert needed.`,
      );
      return new Response(
    ...
    if (resendKey) {
      ...
      console.log(
        "[COST-ALERT-CRON] No RESEND_API_KEY configured. Skipping email. Alert still logged.",
      );
    }
