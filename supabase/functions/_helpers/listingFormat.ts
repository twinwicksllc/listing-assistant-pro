...
/**
 * Converts a plain-text description into self-contained, inline-styled,
 * mobile-safe HTML for eBay's `listingDescription`.
 *
 * Input format: blank-line-separated blocks. Within each block:
 *   - Prose paragraphs render as <p> tags
 *   - "Label: Value" lines (2+ per block) render as <ul><li> spec lists
 *   - Bullet lines (- or * prefix) render as <ul><li>
 *   - Numbered lines (1. or 1) prefix) render as <ol><li>
 *
 * Critical: blank lines (2+) are the *only* layout signal. Without them,
 * plain-text descriptions flatten to a single wall of text on eBay.
 * The Gemini prompt MUST output one blank line between:
 *   - Opening paragraph
 *   - Physical description
 *   - Historical/Why It's Cool section
 *   - "Quick Specs:" header and the spec lines
 *   - Closing/AI disclosure
 *
 * Descriptions that already contain block-level HTML keep their own layout
 * and are only wrapped in the styled container.
 */
export function formatDescriptionHtml(raw: string): string {
  ...
  const rendered = blocks
    .map((block) => {
      // Special handling for "Quick Specs:" header line only
      // Treat it as a prose line that becomes a header paragraph
      if (block.toLowerCase() === "quick specs:") {
        return `<p style="${PARAGRAPH_STYLE}"><strong>Quick Specs:</strong></p>`;
      }
 ...
 * from data the structured call already returned, structured to match the
 * new Gemini prompt format (opening → physical description → why it matters
 * → Quick Specs → closing).
 *
 * Critical: BLANK LINES between sections are required. formatDescriptionHtml()
 * reads them to build proper HTML layout — without them, everything flattens
 * to a single wall of text on eBay.
 */
export function buildFallbackDescription(
  ...
  lines.push("");
  lines.push("Listing generated by ListrAssistr. All details should be verified by the buyer.");
