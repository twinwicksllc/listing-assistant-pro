...
test.describe("Coin Condition Validation (Phase 3)", () => {
  ...
  test.describe("Graded Coin Validation", () => {
    test("accepts valid graded coin (PCGS MS 65)", async ({ page }) => {
      ...
      const gradedButton = page
        .locator('button:has-text("Graded coin")')
        .first();
    ...
    test("accepts valid graded currency (Legacy Currency Grading)", async ({
      page,
    }) => {
    ...
    test("requires grade field for graded coins", async ({ page }) => {
      ...
      const errorBox = page
        .locator('div:has-text("Grade is required")')
        .first();
      await expect(errorBox).toBeVisible({ timeout: 5_000 });
    });
  });
  ...
  test.describe("Raw Coin Validation", () => {
    ...
    test("rejects invalid raw condition tier", async ({ page }) => {
      ...
      const conditionOptions = options.filter(
        (opt) => opt && opt !== "— Select a condition tier —",
      );
  ...
  test.describe("Fallback & Error Recovery", () => {
    test("shows clear error if coin condition required but not provided", async ({
      page,
    }) => {
      // Upload coin photo
      await uploadTestPhoto(page, "coin");
      ...
      if (await publishButton.isDisabled()) {
        ...
        const errorMessage = page
          .locator("text=Coin condition details are REQUIRED")
          .first();
    ...
    test("gracefully handles metadata API failures with fallback", async ({
      page,
    }) => {
