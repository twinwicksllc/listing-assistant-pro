...
export async function login(page: Page, user: TestUser) {
  ...
  if (
    /\/(home|drafts|analyze|dashboard|billing)(\?|$)/.test(
...
export async function uploadTestPhoto(
  ...
  await page
    .getByRole("button", { name: "Process Now" })
...
export async function upgradeSubscription(
  ...
  const planButton = page
    .locator(
      `button:has-text("${plan.charAt(0).toUpperCase() + plan.slice(1)}") ~ button:has-text("Upgrade")`,
    )
    .first();
  ...
  await page.waitForURL(
    (url) =>
      url.hostname.includes("stripe.com") || url.hostname.includes("localhost"),
    { timeout: 10_000 },
  );
