...
> **Verified in code 2026-09-25:** all three are present — `src/v2/components/CategoryHeatMap.tsx` exists (added in PR #613), and `DashboardPage2.tsx` renders `<CategoryHeatMap listings={listings} maxTiles={12} />` directly below the listings section, gated on `planFeatures.hasListingAnalytics` (Pro/Shop), the same flag the other analytics widgets on that page use — it originally used a one-off `currentPlan === "pro" || "shop"` check. Also fixed 2026-09-25: the component read `ebay_category_id` (the drafts-table column) while the dashboard passes eBay listings carrying `categoryId`, so every listing landed in one "unknown" tile; tiles are now labeled with the category leaf name. Covered by `src/test/category-heat-map.test.tsx`. The heat thresholds are simple listing counts (≥10 green, ≥5 amber, otherwise red).
