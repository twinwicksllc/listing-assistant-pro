...
| Q-03 | **Canonical host: apex or `www`?** Now urgent rather than cosmetic — Vercel lists only `www.listrassistr.com`, so the apex may be redirecting to `www`, which would contradict plan §6.1. **Send the Vercel → Domains screen** (A.3a)                                                                                                                                              | Redirect config, canonical URLs, sitemap  | **P1**   |
...
| O-34 | **Send the Vercel → Domains screen**, showing the full domain list, which is primary, and any redirect. Resolves Q-03 and A.3a's apex-versus-`www` ambiguity in one screenshot. Also open **Deployment Settings → the 1 Recommendation** — if Vercel is recommending a different apex A value, that is where it appears                                         | —          | **P1**   |
