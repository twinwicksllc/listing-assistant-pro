...
**Every `LA-`/`BK-` hyphenated SKU is rejected by eBay's Inventory API, and one is enough to break inventory enumeration for the whole account (2026-09-25, PR #626).** eBay allows only alphanumeric SKUs (errorId 25707). A single stored bad SKU makes bulk `GET /sell/inventory/v1/offer` fail, so `ebay-listings` falls back to the Trading API (`GetMyeBaySelling`) on every sync — and that path returns no category, which is why `user_active_listings.category_id` was NULL on every row until #623 added a GetItem backfill on the fallback. The root cause was not the SKU format but a missing field: `useAnalyzePublish` never sent `userId` to `create_draft`, so `generateDraftSku` always skipped the sequential `LA#####` path for its random `LA-<hex>` fallback. Earlier fixes (March 2026) worked around the `/offer` error instead of this. #626 fixed generation, but ~805 existing listings still carry hyphenated SKUs, so `/offer` keeps failing until they're migrated — the Trading API fallback is the de facto path for now. Don't reattempt a format-only fix.
