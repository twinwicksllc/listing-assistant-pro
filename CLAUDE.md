...
**eBay splits its APIs across two hosts, and two known bugs were found on this surface (2026-09-14); the host bug is still open as of 2026-09-25, the scope bug is partly fixed.** Identity and Finances live on **`apiz.ebay.com`**, not `api.ebay.com` — `ebay-publish/constants.ts` gets this right (`IDENTITY_API_PROD`/`_SANDBOX`), but `ebay-user/index.ts` builds its own `apiBase` as `api.ebay.com` and so 502s on every `/commerce/identity/v1/user/` call; it's invoked from both `src/pages/DashboardPage.tsx` and `src/v2/pages/DashboardPage2.tsx`. Still unfixed at `ebay-user/index.ts:117` — #607 only added a retry around it. (`FREE_TIER_IMPLEMENTATION_PLAN.md` calls `ebay-user` "confirmed working in production"; check its function logs before trusting either claim.)
