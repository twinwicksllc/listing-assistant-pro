...
**The stale-ID disease turned out to be systemic across the coin/bullion vertical, not confined to one list (2026-09-01).** Beyond the tool-schema description above (already clean), the same class of bug — dead IDs, and worse, _live leaves silently reassigned to a different domain_ (`40150` now resolves to Action Figures, not Roosevelt Dime) — was found and fixed in `_helpers/domainPrompts.ts`'s `buildCoinBullionPrompt()` (the actual prompt text Gemini sees), `src/lib/ebayCategoryMap.ts` (a frontend breadcrumb map `CategoryConfirmDialog.tsx` checks _before_ any live verification — the most dangerous instance, since a stale hit reports "valid" with no real check ever firing), and `analyze-item`'s `COINS_PAPER_MONEY_IDS` and `resolveDomainFallbackCategory()`. See `todo.md`'s "fix the stale/wrong-domain eBay coin-category IDs" entry for the full replacement table and three more instances found but deliberately not fixed yet (`ebay-publish/publish-helpers.ts`'s `HARDCODED_COIN_CATEGORY_IDS`, `src/types/listing.ts`'s `COIN_CATEGORY_IDS`/`BULLION_CATEGORY_IDS`, and `_helpers/suggestedCategories.ts`'s breadcrumb map — each a separate file/subsystem warranting its own audit). Every other domain (trading cards, jewelry, electronics, etc.) has zero hardcoded category IDs and cannot have this exact bug.
