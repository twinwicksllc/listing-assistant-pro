...
4. Re-verify: trigger `inventory-sync-cron` a few times, confirm it cycles through *different* users each time (or exhausts the pool and stops returning the same one), then trigger `competitor-prices-cron` and confirm `batchSize` grows as more users' inventories land in `user_active_listings`.
