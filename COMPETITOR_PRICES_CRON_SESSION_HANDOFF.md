...
**Two sub-questions (never answered — see the 2026-08-25 update above; the fix made them moot for the starvation symptom):**
...
**Fix (implemented — option (a) shipped; see the 2026-08-25 update above; originally "proposed, not yet implemented"):** persist a "last sync attempt" signal that's written **unconditionally**, regardless of how many listings were found, so a legitimately-zero-listings account only occupies a slot once every `INVENTORY_STALE_MS` (6h) like everyone else. Two ways to do this, pick one:
...
Recommend (a) for simplicity unless there's a reason to prefer (b). _(Option (a) was chosen and shipped.)_
...
## Later changes to these files

- **2026-09-25, PR #623:** `supabase/functions/_helpers/ebayInventorySync.ts` now backfills `user_active_listings.category_id` via Trading API `GetItem` on the Trading API fallback path, and never overwrites a stored `category_id` with null.

## Next steps (in order)

_Status updated 2026-09-25. Only step 3 is recorded as done; the outcome of the others was never written down here._
